...
const MedicalProtocolsApp = () => {
  ...
  // --- MRI Search helpers ---
  const allMriProtocols = Object.values(mriProtocolsData);
  const mriSearchQueryTrimmed = mriSearchQuery.trim();
  const mriSearchQueryLower = mriSearchQueryTrimmed.toLowerCase();
  const mriFilteredResults = mriSearchQueryLower.length >= 1 ? allMriProtocols.filter(p => {
    const titleMatch = p.title.toLowerCase().includes(mriSearchQueryLower);
    const titleArMatch = p.titleAr && p.titleAr.includes(mriSearchQueryTrimmed);
    const keyMatch = p.key.toLowerCase().includes(mriSearchQueryLower);
    return titleMatch || titleArMatch || keyMatch;
  }) : [];

  const handleMriSearchSelect = (proto) => {
  ...
  // Render search bar as inline JSX (not a component) to prevent input losing focus
  const renderMriSearchBar = () => (
